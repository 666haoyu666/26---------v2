...
static motor_drv_status_t group_apply(motor_op_t op);
...
static motor_drv_status_t group_apply(motor_op_t op)
{
    ...
    for (i = 0U; i < MOTOR_DRV_MAX_NUM; i++) {
        ...
        done_cnt++;
        if ((ret != MOTOR_DRV_OK) && (first_err == MOTOR_DRV_OK)) {
            first_err = ret;
        }
    }
    ...
    uint32_t count)
{
    ...
    for (i = 0U; i < count; i++) {
        ...
        motor_drv_status_t ret = dev->pf_set_rps(dev, targets[i].rps);
