/**
 * @file    bsp_wrapper_imu.c
 * @brief   IMU Wrapper：能力表持有、生命周期门禁与安全转发
 * @note    - 不含器件、HAL、RTOS知识，全部具体绑定在Adapter完成
 *          - 契约限定注册在内核启动前、read在单一任务，故不加锁
 */
...
platform_err_t bsp_imu_read(bsp_imu_data_t *data)
{
    ...
    /* 失败时不修改data由Adapter回调契约保证 */
    return s_dev.pf_read(data);
}
